...
export default function WindowEdit({
  ...
  const [firstName, setFirstName] = useState(targetItem.firstName);
  const [lastName, setLastName] = useState(targetItem.lastName);
  const [patronymic, setPatronymic] = useState(targetItem.patronymic);
  const [email, setEmail] = useState(targetItem.email);
  const [login, setLogin] = useState(targetItem.login);
  const [messageError, setMessageError] = useState("");
  useEffect(() => updateTargetItem(targetItem), [targetItem]);
  const updateTargetItem = (targetItem) => {
    setFirstName(targetItem.firstName);
    setLastName(targetItem.lastName);
    setPatronymic(targetItem.patronymic);
    setEmail(targetItem.email);
    setLogin(targetItem.login);
    setMessageError("");
  };
  const saveItem = (event) => {
    ...
    if (resultFindEmail !== undefined && email !== targetItem.email) {
      setMessageError(`Пользователь с таким e-mail уже существует!`);
    }
    if (resultFindLogin !== undefined && login !== targetItem.login) {
      setMessageError(`Пользователь с таким логином уже существует!`);
    }
    if (
      (resultFindLogin === undefined && resultFindEmail === undefined) ||
      (resultFindLogin !== undefined &&
        resultFindEmail === undefined &&
        login === targetItem.login) ||
      (resultFindLogin === undefined &&
        resultFindEmail !== undefined &&
      ...
      const filterArr = array.filter((item) => item.login !== targetItem.login);
      const newArray = [newItem, ...filterArr];
  ...
  return (
    <form onSubmit={saveItem} className="form-box">
      <div className="form-box__title">
        <p className="form-box__title__p">Редактирование пользователя</p>
        <button
